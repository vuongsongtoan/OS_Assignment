...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    /* Index for caller->mswp */
    int idx = 0;
    
    struct memphy_struct *a = (struct memphy_struct *)caller->mswp;
    for(idx = 0; idx < PAGING_MAX_MMSWP; idx++)
    {
       if(a + idx == caller->active_mswp)
          break;
    }
    ...
    {
       struct memphy_struct **b = caller->mswp;
       for(int idx = 0; idx < PAGING_MAX_MMSWP; idx++)
       {
          struct memphy_struct *c = (struct memphy_struct *)b;
          if(MEMPHY_get_freefp(c + idx, &swpfpn) == 0)
          {
             /* Do swap frame from MEMRAM to MEMSWP and vice versa*/
             /* Copy victim frame to swap */
             __swap_cp_page(caller->mram, vicfpn, c + idx, swpfpn);
             caller->active_mswp = c + idx;
             break;
          }
       }
    }
    ...
    __swap_cp_page(a + tgtswp_typ, tgtfpn, caller->mram, vicfpn);
    
    /* Free frame in swap including tgtfpn that has been copied to RAM */
    MEMPHY_put_freefp(a + tgtswp_typ, tgtfpn);

    /* Update page table, set swap for index vicpgn */
    pte_set_swap(&caller->mm->pgd[vicpgn], idx, swpfpn);

    /* Update its online status of the target page */
    /* Set online frame for idx pgn with vicfpn which has moved to swap */
